...
#### STEP 18 최종 검증 중 보완 — 수집 범위 문구 / Gmail Plain Text (새 STEP 아님)

발견한 문제 (사용자 확인):
- 제한 사항 "검색 결과 한 페이지의 일부 공고만 사용했습니다."가 모호함 — 실제 로직은 검색결과 1페이지에서 `MAX_JOBS = 5`, 상위 5건만 수집
- 사용자가 본 Gmail 내용에 `2\.`, `\--------------------`, `2026-07-01 \~ 2026-09-21`, `\- 경력`, `[https://URL](https://URL)` 같은 Markdown 잔여 문자가 보였음

원인 확인 (네트워크 없이):
- 현재 `build_email_text()`가 만드는 본문에는 역슬래시 0개, `[http` 0개. MIME은 `text/plain; charset="utf-8"`(base64)이고, 발송 형태로 조립한 뒤 복원해도 원래 본문과 같음
- 보인 문자들은 Gmail 화면(일반 텍스트 속 URL이 자동 링크로 표시됨)을 Markdown으로 바꾸는 곳에 복사할 때 생기는 전형적인 이스케이프 형태로 추정 (메일 원본은 직접 확인하지 못함)
- 그래도 요청대로 Gmail renderer에서 방어적으로 정리하도록 보완

보완 (수정 파일: `src/reporter.py`, `src/notifier.py` — main·수집·Gemini·history·workflow 수정 없음):
- 제한 사항 문구 (Slack·Gmail·보고서 공통 `run_summary["limitations"]`):
  - "검색 결과 한 페이지의 일부 공고만 사용했습니다." → "JobKorea 검색결과 1페이지 전체가 아니라 상위 5건만 수집했습니다."
  - "현재 5건 결과를 전체 채용시장 경향으로 일반화하면 안 됩니다." → "현재 5건은 검색결과 1페이지 전체나 전체 채용시장을 대표하지 않습니다."
  - 페이지 전체 공고 수는 수집 결과에 담지 않으므로 숫자(예: 20건)를 쓰지 않음
- Gmail: `build_email_text()`가 마지막에 본문 전체를 `_plain()`으로 한 번 더 정리
  - `[표시문구](URL)` → URL 한 번만, `\.` `\-` `\~` 등 Markdown 이스케이프의 역슬래시 제거, `**` 제거, 줄 맨 앞 `#` 제거
- Slack·Markdown 보고서 형식은 그대로 유지

검증 (외부 요청 0회, 가짜 Gemini 응답에 `\-`·`\.`·`\~`·`[URL](URL)`·`**`·`#`를 일부러 넣어 확인):
- Gmail: `\-`·`\.`·`\~`·역슬래시 없음, `[http` 링크 없음(링크는 URL 한 번만), `**`·줄 맨 앞 `#` 없음, 공고 URL 각 1회,
  "2. 공고별 상세 정보 / ----", "지원 시작일 범위: 2026-07-01 ~ 2026-09-21" 그대로
- 새 제한 사항 두 문장이 Slack·Gmail·보고서에 모두 있고, 옛 문구는 세 곳 모두 없음, 제한 사항 목록 동일
- 이전 검증(출력 정합성 CASE A/B, JobKorea 연결 오류 CASE 1~5) 다시 실행 — 모두 통과
- history CSV·보고서 체크섬 변경 없음

#### STEP 18 최종화 전 변경 — 수집 건수 5건 → 최대 20건 (새 STEP 아님)

변경 (수정 파일: `main.py`, `src/reporter.py` — crawler·preprocess·gemini_client·workflow 수정 없음):
- `main.py`: `MAX_JOBS = 5` → **`MAX_JOBS = 20`** (JobKorea 검색결과 1페이지 안에서 최대 20건, 페이지네이션·상세 페이지 수집 없음)
  - `GEMINI_MAX_JOBS = 2` 그대로 (Gemini 호출 최대 2회 유지), timeout·재시도·User-Agent 정책 그대로
  - `build_run_summary(..., max_jobs=MAX_JOBS)`로 설정값 전달
- `src/reporter.py`: `collection_scope_text(total, max_jobs)` 추가 — 수집 범위 문장을 실제 수집 건수와 설정값으로만 만든다
  - 설정만큼 수집: "JobKorea 검색결과 1페이지에서 상위 20건을 수집했습니다. (설정 최대 20건)"
  - 설정보다 적게 수집: "JobKorea 검색결과 1페이지에서 12건을 수집했습니다. (설정 최대 20건보다 적게 수집됨)"
  - 대표성 제한: "현재 N건은 검색결과 1페이지 범위의 결과이며, 전체 채용시장을 대표하지 않습니다."
  - 이전 문구 "1페이지 전체가 아니라 상위 N건만"은 제거 — 20건을 수집하면 1페이지 전체일 수도 있기 때문
    (07-A에서 저장한 검색 결과 HTML의 1페이지 카드는 20개였음. 1페이지 전체 공고 수는 수집 결과에 담지 않으므로 문구에 쓰지 않음)
  - 보고서 1장 "수집 기준"도 같은 문장 사용 → Slack·Gmail·보고서 모두 같은 수집 범위 의미

검증 (외부 요청 0회 — 네트워크 차단, history CSV 읽기만, 보고서 덮어쓰기 없음):
- fixture 5건 / 10건 / 20건 / 12건(설정보다 적게 수집): 모두 통과
  - 수집 범위 문장이 실제 건수·설정값 기준, Slack·Gmail·보고서 세 곳 동일, 대표성 문장 세 곳 동일, "상위 5건" 고정 문구 없음
  - 헤더 "현재 수집된 N건 기준", 공고 N건 모두 표시, 공고 URL 각 1회, Gemini 호출(가짜 client) 최대 2회
  - 문자 수 (보고서 / Slack / Gmail): 5건 3528 / 1918 / 2575, 10건 4249 / 2933 / 3649, 20건 5654 / 4967 / 5785
- 07-A에서 저장한 실제 검색 결과 HTML: 카드 20개 → `max_jobs=20`으로 20건 추출, 결측 0, `job_url` 중복 0, 앞 5건은 기존 history와 같음
- 이전 검증(출력 정합성, 연결 오류) 및 `main(dry_run=True)` 다시 실행 — 모두 통과, 외부/저장 함수 호출 0회

다음 실제 실행에서 예상되는 점:
- history가 기존 5건에서 최대 20건으로 늘어나고, 처음에는 신규 공고가 최대 15건으로 잡힐 수 있음 (history 중복 판별 로직은 그대로)
- 20건이면 Slack 메시지가 약 5천 자로 길어짐 (Slack text 한도 안), 사용자 화면에서 가독성 확인 필요
